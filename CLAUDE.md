...
**Embedding in the newsletter** (note: GitHub Pages serves from `docs/`, so `docs/` is stripped from the public URL): each run writes a ready-to-paste HTML snippet — `snapshots/featured-pet-latest.html` (newest) and `snapshots/featured-pet-YYYY-MM-DD-{am,pm}.html` (stable per-edition). The snippet contains the PNG wrapped in an `<a>` to the pet's adoption profile (a wrapped `<img>` IS clickable in email, even though text drawn inside the PNG is not) plus a real "Meet [Name] →" text link below it. Use the dated per-edition URL so the image doesn't change after send. The raw PNGs are also at `snapshots/featured-pet-latest.png` / `...-{am,pm}.png`, and `snapshots/featured-pet.json` exposes the featured pet's name/shelter/adoption URL for the newsletter automation.
